...
/**
 * SRM 144 - DIV 1 - 250 pts
 * Practice success - 24/05/13
 *
 * Problem Statement
     
...
public class BinaryCode250 {
    public String[] decode(String message) {
        ...
        // Assuming P[0] = 0
        char[] PA = new char[message.length()];
        // a represents P[i-1]
        int a=0;
        ...
        int b=0;
        PA[0]='0';
        for(int i=1; i<message.length()&&!exit;i++) {
            if (i>0)
                a=(int)(PA[i-1]-'0');
            if (i>1)
                b=(int)(PA[i-2]-'0');
            val=(int)(message.charAt(i-1)-'0')-a-b;
            if(val<0 || val >1)
                exit=true;
            else PA[i]=(char)(val+'0');
        }
        if (exit) result[0]=new String("NONE");
        else result[0]=String.valueOf(PA);

        // Assuming P[0] = 0
        char[] PB = new char[message.length()];
        // a represents P[i-1]
        a=1;
        ...
        for(int i=1; i<message.length()&&!exit;i++) {
            ...
            if(val<0 || val >1)
                exit=true;
            else PB[i]=(char)(val+'0');
// 		System.out.println("Q[i]="+(int)(message.charAt(i)-'0')+", P[i-1]="+a+", P[i-2]="+b+", => P[i]="+val);
        }
        if (exit) result[1]=new String("NONE");
        else result[1]=String.valueOf(PB);
